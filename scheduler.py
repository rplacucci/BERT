...
class LinearWarmupLinearDecay(Scheduler):
    """
    BERT (Devlin et al.) learning rate schedule:
      1) Linear warmup to lr_max over warmup_steps
      2) Linear decay from lr_max to 0 over total_steps - warmup_steps
    """
    def __init__(self, optimizer, lr_max, warmup_steps, total_steps):
        super().__init__(optimizer)
        self.lr_max = lr_max
        self.warmup_steps = warmup_steps
    ...
    def step(self):
        ...
        if self.step_num <= self.warmup_steps:
            ...
            scale = max(0.0, (self.total_steps - self.step_num) / (self.total_steps - self.warmup_steps))
